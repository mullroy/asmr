...
pub struct VerusVerifier(VerusEngine);

impl VerusVerifier {
  pub async fn new(config_path: &Path) -> anyhow::Result<VerusVerifier> {
    Ok(VerusVerifier(VerusEngine::new(serde_json::from_reader(File::open(config_path)?)?).await?))
  }
}

#[async_trait]
impl UnscriptedVerifier for VerusVerifier {
  ...
  async fn verify_and_wait_for_send(&mut self) -> anyhow::Result<()> {
    ...
    if !cfg!(test) {
      print!("You will receive {} atomic units of VRSC. Continue (yes/no)? ", deposit);
      std::io::stdout().flush().expect("Failed to flush stdout");
      let mut line = String::new();
      std::io::stdin().read_line(&mut line).expect("Couldn't read from stdin");
      if !line.to_lowercase().starts_with("y") {
        anyhow::bail!("User didn't confirm VRSC amount");
      }
    }
