...
use crate::{
  ...
  coins::{UnscriptedClient, ScriptedVerifier, vrsc::engine::*}
};
...
impl VerusClient {
  pub async fn new(config_path: &Path) -> anyhow::Result<VerusClient> {
    Ok(VerusClient{
      engine: VerusEngine::new(serde_json::from_reader(File::open(config_path)?)?).await?,
      deposited: false,
      #[cfg(test)]
      refund_seed: [0; 32]
    })
...
#[async_trait]
impl UnscriptedClient for VerusClient {
