    ...
    Fut: Future<Output = anyhow::Result<bool>>
{
  ...
  let unscripted: PathBuf = "config/arrr.json".to_string().into();
  ...
  let hosts_verifier: AnyUnscriptedVerifier = ArrrVerifier::new(&unscripted).await.expect("Failed to create BTC verifier").into();

  let mut client: AnyUnscriptedClient = ArrrClient::new(&unscripted).await.expect("Failed to create ZCash client").into();
  ...
  if host_test {
    ...
    let client = ArrrClient::new(&unscripted).await.unwrap();
